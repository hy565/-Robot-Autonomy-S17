...
class HerbEnvironment(object):
	...
	bodies =  self.robot.robot.GetEnv().GetBodies()
        config = [0] * len(self.robot.robot.GetActiveDOFIndices())
	lower_limits, upper_limits = self.robot.robot.GetActiveDOFLimits()
	collision = True
	while(collision):
		config = numpy.array(numpy.random.uniform(lower_limits,upper_limits,len(self.robot.robot.GetActiveDOFIndices())))
		self.robot.robot.SetActiveDOFValues(config)
		collision = (self.robot.robot.CheckSelfCollision() or self.robot.robot.GetEnv().CheckCollision(bodies[1],bodies[0]))
